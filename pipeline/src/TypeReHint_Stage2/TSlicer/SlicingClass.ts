...
import { DataType, CodeData, FUNCTION_KINDS, BLOCK_KINDS, FILE_DEFINED, ARRAY_OPERATIONS, SlicedData, IDENTIFIER_KINDS } from "../utils/typeDefined";
import { AnalysizerRule } from "./OtherRule";
import { ThirdPackageLoader } from "./ThirdPackageClass";
...
export class TSlicer {
    ...
    constructor(filePath: string, projectDataPath: string) {
        ...
        this.packageLoader = new ThirdPackageLoader(importInfos);
    ...
    public Slicing(data: CodeData) {
        ...
        if (data.dataType === DataType.Var) {
            ...
            let rootNode = this.getRootNode(node);
            let functionName = "";
            if (node.getParent().getKind() === SyntaxKind.FunctionDeclaration || node.getParent().getKind() === SyntaxKind.MethodDeclaration) {
                functionName = node.getParent().getName() == undefined ? data.FunctionName : node.getParent().getName();
            }
            else {
            ...
            if (functionName == "") {
                if (ans.size == 0) {
                    ans.add(node.getParent().getText());
                }
                let funcNodeCode = node.getParent().getText();
                ...
                slicingFuncName = functionName;
                ans.add(node.getParent().getKind() != SyntaxKind.ArrowFunction && node.getParent().getName() == undefined ? node.getParent().getText().replace("function", "function " + functionName) : node.getParent().getText());
                this.projectDataLoader.delFunction(functionName);
                let isMemberMethod = node.getParent()?.getKind() === SyntaxKind.MethodDeclaration;
                let res = this.getBlockNameVar(rootNode, targetName, data.filePath, false, functionName, true);
                res.forEach(item => ans.add(item));
                const project = new Project();
                const sourceFile = project.addSourceFileAtPath(data.filePath);
    ...
    public trackIdentifier2(expression: any, targetName: string, filePath: string) {
        ...
        if (startKindType === SyntaxKind.PropertyAccessExpression) {
            if (tempNode.getName() == targetName) {
                ...
                if (tempNode.getName() == 'filter' && tempNode.getParent().getKind() === SyntaxKind.CallExpression) {
                    let args = tempNode.getParent().getArguments();
            ...
            res.add(tempNode.getParent().getText());
        }
        let isObject = false;
        while (tempNode) {
            ...
            if (tempKind == SyntaxKind.ExpressionStatement) {
                    ...
                    identifys.forEach((item: any) => {
                        if (item.getText() == targetName) {
                            ...
                            if (tempData2) {
                                ...
                                res.add(item.getParent().getText());
                            }
                        }
                    });
                    break;
                ...
                res.add(expression.getParent().getText());
                break;
            }
    ...
    public trackCallExpression2(expression: any, filePath: string, isFunc: boolean = false, isParam: boolean = false) {
        ...
        let fDeined = this.getNearestFunction(rootNode, tempNode.getExpression().getText());
        if (fDeined.size > 0) {
            console.log(fDeined);
            ...
            if (data0.size == 0) {
                let tpPackageData = this.packageLoader.getFunctionByName(tempNode.getExpression().getText());
                tpPackageData.forEach(d => { res.add(d); });
                let data = this.projectDataLoader.findFunction(tempNode);
                data.forEach(item => res.add(item));
            }
            if (res.size == 0) {
                logger.warning(`Definition not found:${tempNode.getText()}`);
            }
        }
        while (tempNode) {
            let tempKind = tempNode.getKind();
            if (tempKind === SyntaxKind.CallExpression) {
                ...
                if (callFunctionName.includes(".")) {
                    ...
                    let varDec = this.GetDeclarationNode(varName, tempNode.getParent());
                    if (varDec != null) {
            ...
            if (tempKind === SyntaxKind.VariableDeclaration) {
                if (isFunc) {
                    let tempSet = this.trackFparam(isCallExpre ? expression : expression.getParent(), filePath);
        ...
        if (isFunc) {
            let tempSet = this.trackFparam(isCallExpre ? expression : expression.getParent(), filePath, isParam);
            tempSet.forEach(item => res.add(item));
        }
        let callFuncName = isCallExpre ? expression.getExpression().getText() : expression.getParent().getExpression().getText();
        ...
        res.add(isCallExpre ? expression.getText() : expression.getParent().getText());
        return res;
    }
    ...
    public getBlockNameVar(root: any, varName: string, filePath: string, isparam: boolean = false, paramFunctionName: string = "", alreadyGetDeclar: boolean = false) {
        ...
        Identifiers.forEach((currentNode: any) => {
            if (currentNode.getKind() === SyntaxKind.Identifier &&
                ...
                currentNode.getParent().getKind() === SyntaxKind.CallExpression && this.isArg(currentNode)) {
                let expr = currentNode.getParent().getExpression();
        ...
        if (!isGetVarDeclar) {
            if (root.getKind() != SyntaxKind.SourceFile && (root.getParent().getKind() === SyntaxKind.FunctionDeclaration || root.getParent().getKind() === SyntaxKind.MethodDeclaration) && !isparam) {
                const args = root.getParent()?.getParameters();
    ...
    public trackFparam(expression: any, filePath: string, isParam: boolean = false) {
        let res = new Set();
        logger.debug(`expr parent:${expression.getParent().getText()}`);
        ...
        let expressionText = exprParent && exprParent.getKind() === SyntaxKind.VariableDeclaration ? exprParent.getText() : expression.getText();
    ...
    public GetDeclarationNode(targetName: string, rootNode: any, needTrackRoot: boolean = true) {
        ...
        while (tempNode) {
            ...
            for (const item of varLists) {
                if (this.isNameEquip(targetName, item) && item.getType().getText() != "mask") {
            ...
            for (const p of params) {
                ...
                if (param.getName() === targetName && !param.getType().getText().includes(mask)) {
    ...
    public getKnowledgeDefined(callExpreNode: any) {
        ...
        if (ans.size == 0) {
            let fAns = this.packageLoader.getFunctionByName(callExpreNode.getExpression().getText());
            fAns.forEach(item => {
                ans.add(item);
            });
        }
