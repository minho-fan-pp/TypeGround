...
export function initializeProjectWithConfig(projectPath: string): {
    ...
    if (!tsConfigPath) {
        ...
        const allFiles = findTypeScriptFiles(projectPath);
        logger.info(`Found ${allFiles.length} TypeScript files in ${projectPath}`);
        const project = new Project();
        allFiles.forEach((file: string) => project.addSourceFileAtPath(file));
        return { project: project, tsConfigPath: null };
    }
